...
for r in $region_list
  do
   ...
   aws s3 sync ./global-s3-assets/ s3://$DIST_OUTPUT_BUCKET-$r/$SOLUTION_NAME/$VERSION/ --delete --acl bucket-owner-full-control
   aws s3 sync ./regional-s3-assets/ s3://$DIST_OUTPUT_BUCKET-$r/$SOLUTION_NAME/$VERSION/ --delete --acl bucket-owner-full-control
done
